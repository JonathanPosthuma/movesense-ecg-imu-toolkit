...
// LED blinking period in start logging mode
...
void HrWakeupApp::startHRLogging()
{
    DEBUGLOG("startHRLogging()");

    // Configure DataLogger to record HR and start it. Indicate start with blinking LED for a couple of seconds
    WB_RES::DataLoggerConfig ldConfig;
    WB_RES::DataEntry entry;
    entry.path = "/Meas/HR";  // Change path to log HR data
    ldConfig.dataEntries.dataEntry = wb::MakeArray<WB_RES::DataEntry>(&entry, 1);

    // Set new config for logging HR data
    asyncPut(WB_RES::LOCAL::MEM_DATALOGGER_CONFIG(), AsyncRequestOptions::ForceAsync, ldConfig);

    // Subscribe to memory full notification
    // NOTE: Modified to ignore full memory and continue logging
    asyncSubscribe(WB_RES::LOCAL::MEM_LOGBOOK_ISFULL(), AsyncRequestOptions::ForceAsync);

    // Start logging indication with LED blinking for a few seconds
    ...
    // Start Logging (continue even if memory is full)
                              ...
                              const whiteboard::Value& result)
{
    switch(resourceId.localResourceId)
    {
        ...
        case WB_RES::LOCAL::MEM_LOGBOOK_ENTRIES::LID:
        {
            ...
            uint32_t logsInLogbook = logbookEntries.elements.size();
            DEBUGLOG("Logbook has %u logs.", logsInLogbook);
            if (logsInLogbook == 0)
            {
                DEBUGLOG("Logbook was empty, starting HR logging");
                startHRLogging();
            }
            else
            {
