...
winlogger::winlogger():
    ...
    mBatteryLevelResource(wb::ID_INVALID_RESOURCE),
    mBatteryLevelRef(0),
    mBatteryLevelPending(false)
{    
}
...
enum Commands
{
    ...
    QA_START_ECG_125 = 7, // Start QA ECG preview at 125 Hz
    QA_STOP          = 8,  // Stop QA ECG preview
    GET_BATTERY_LEVEL = 9  // Query /System/Energy/Level and return %
};
...
void winlogger::handleIncomingCommand(const wb::Array<uint8> &commandData){
    ...
    switch (cmd)
    {
        case Commands::GET_BATTERY_LEVEL:
        {
            // Query battery level via WB resource and reply on DATA characteristic.
            // This is intentionally a NEW command to avoid interfering with HELLO reset behaviour.
            DEBUGLOG("GET_BATTERY_LEVEL received. reference: %d", reference);

            mBatteryLevelRef = reference;
            mBatteryLevelPending = true;

            // Resolve resource id once
            if (mBatteryLevelResource == wb::ID_INVALID_RESOURCE)
            {
                getResource("/System/Energy/Level", mBatteryLevelResource);
            }

            asyncGet(mBatteryLevelResource, AsyncRequestOptions::ForceAsync);
            return;
        }
        case Commands::HELLO:
        {
        ...
        case Commands::STOP_LOGGING:
        {
            ...
            return;
                ...
                case Commands::QA_START_ECG_125:
        {
            DEBUGLOG("QA_START_ECG_125 received. reference: %d", reference);

            // Idempotent: if QA already active, just ACK success
            if (mQaECGActive && mQaEcgResId != wb::ID_INVALID_RESOURCE) {
                uint8_t resp[] = { COMMAND_RESULT, reference, 0x00 };
                WB_RES::Characteristic dataCharValue;
                dataCharValue.bytes = wb::MakeArray<uint8_t>(resp, sizeof(resp));
                asyncPut(mDataCharResource, AsyncRequestOptions::Empty, dataCharValue);
            ...
            // Start lightweight QA ECG at 125 Hz using existing subscribe machinery
            startQaEcg(reference, mQaEcgArraySize);
        ...
        case Commands::QA_STOP:
        {
            DEBUGLOG("QA_STOP received. reference: %d", reference);

            // Stop QA ECG if active (idempotent)
            stopQaEcg(reference);
                                      ...
                                      const wb::Value& rResultData)
{
    ...
    // Handle battery level GET response (custom GATT command)
    if (mBatteryLevelPending && (resourceId == mBatteryLevelResource))
    {
        mBatteryLevelPending = false;

        // Build response: [COMMAND_RESULT, ref, status_lo, status_hi, level]
        // We use status as little-endian uint16 (e.g., 200 OK -> 0xC8 0x00)
        uint8_t resp[5] = { COMMAND_RESULT, mBatteryLevelRef, 0x00, 0x00, 0x00 };

        uint16_t status = static_cast<uint16_t>(resultCode);
        resp[2] = static_cast<uint8_t>(status & 0xFF);
        resp[3] = static_cast<uint8_t>((status >> 8) & 0xFF);

        if (resultCode >= 400)
        {
            // error: keep level at 0
            DEBUGLOG("GET_BATTERY_LEVEL failed. resultCode: %d", resultCode);
        }
        else
        {
            // /System/Energy/Level is expected to be an integer percent-like level
            uint8_t level = 0;
            // convertTo<uint8_t> works for WB integer payloads
            level = rResultData.convertTo<uint8_t>();
            resp[4] = level;
            DEBUGLOG("GET_BATTERY_LEVEL OK. level: %u", level);
        }
...
void winlogger::unsubscribeAllStreams()
{
    ...
    // Also clear QA ECG bookkeeping so state stays consistent
    mQaECGActive = false;
    mQaEcgResId = wb::ID_INVALID_RESOURCE;
    mQaEcgClientRef = 0;
}
...
// Start a lightweight QA ECG subscription at 125 Hz and forward via DATA char
void winlogger::startQaEcg(uint8_t clientRef, uint16_t arraySize /*=32*/)
{
    (void)arraySize; // Reserved for future use if ArraySize can be configured via WB
    ...
    // Reuse existing SUBSCRIBE machinery to subscribe to /Meas/ECG/125/mV
    const char* kEcg125Path = "/Meas/ECG/125/mV";
    DataSub* pDataSub = getFreeDataSubSlot();
    if (pDataSub == nullptr) {
        DEBUGLOG("startQaEcg: No free DataSub slot");
        return;
    }
    ...
    dataSub.subStarted = true;
    dataSub.subCompleted = false;
    dataSub.clientReference = clientRef;

    // Store path text if the struct has room (optional)
    if (sizeof(dataSub.resourcePath) > 0) {
        memset(dataSub.resourcePath, 0, sizeof(dataSub.resourcePath));
        // Safe copy with truncation
        strncpy(dataSub.resourcePath, kEcg125Path, sizeof(dataSub.resourcePath) - 1);
    }

    getResource(kEcg125Path, dataSub.resourceId);
    asyncSubscribe(dataSub.resourceId, AsyncRequestOptions::ForceAsync);

    // Save QA state
    mQaECGActive   = true;
    mQaEcgResId    = dataSub.resourceId;
    mQaEcgClientRef = clientRef;

    DEBUGLOG("startQaEcg: Subscribed to %s (clientRef=%u)", kEcg125Path, clientRef);
}

// Stop the lightweight QA ECG subscription
void winlogger::stopQaEcg(uint8_t /*clientRef*/)
{
    if (!mQaECGActive || mQaEcgResId == wb::ID_INVALID_RESOURCE) {
        // Already stopped
        mQaECGActive = false;
    ...
    // Find the matching DataSub and unsubscribe
    DataSub* pDataSub = findDataSub(mQaEcgResId);
    if (pDataSub != nullptr) {
        asyncUnsubscribe(pDataSub->resourceId);
        pDataSub->resourceId     = wb::ID_INVALID_RESOURCE;
        pDataSub->clientReference = 0;
        ...
        if (sizeof(pDataSub->resourcePath) > 0) {
            memset(pDataSub->resourcePath, 0, sizeof(pDataSub->resourcePath));
        }
    }
    ...
    DEBUGLOG("stopQaEcg: QA ECG unsubscribed and state cleared");
}
