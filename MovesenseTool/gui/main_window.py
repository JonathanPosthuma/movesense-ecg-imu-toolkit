...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        # Create per-sensor timers for the "Found" state.
        self.found_timers = {}
        for i in range(len(SENSOR_LIST)):
            timer = QtCore.QTimer(self)
            timer.setSingleShot(True)
            # Use a lambda default argument to capture the sensor index.
    ...
    def toggle_sensor_found(self, sensor_index):
        ...
        self.log_message(f"Sensor {SENSOR_LIST[sensor_index]} toggled to Found.")
    ...
    def handle_found_timeout(self, sensor_index):
        ...
        if current_status == "found":
            ...
            self.log_message(f"Sensor {SENSOR_LIST[sensor_index]} timed out; reverting to Pending.")
